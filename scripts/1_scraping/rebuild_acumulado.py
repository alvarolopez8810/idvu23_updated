...
def main():
    ...
    parser.add_argument('--tournaments', default='', help='IDs separados por coma')
    ...
    if args.tournaments:
        print(f'  Torneos     : {args.tournaments}')
    print('=' * 70)
    ...
    for i, (chunk_from, chunk_to) in enumerate(chunks):
        ...
        if args.tournaments:
            cmd += ['--tournaments', args.tournaments]

        result = subprocess.run(cmd)

        if result.returncode != 0:
            print(f'  AVISO: chunk {chunk_from}/{chunk_to} falló (returncode={result.returncode})')
    ...
    if failures:
        ...
        for f, t in failures:
            print(f'  - {f} a {t}')
        print('Puedes re-ejecutar con esas fechas manualmente.')
        sys.exit(1)
    else:
        print(f'Reconstruccion completada: {len(chunks)} chunks procesados correctamente.')
        sys.exit(0)
